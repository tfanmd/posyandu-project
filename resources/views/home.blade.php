                ...
                @forelse($jadwal_terdekat as $jadwal)
                    <div class="col-md-4">
                        <div class="card shadow-sm border-0 h-100">
                            <div class="card-body">
                                <span
                                    class="badge bg-success mb-2">{{ \Carbon\Carbon::parse($jadwal->waktu_mulai)->translatedFormat('d F Y') }}</span>
                                <h5 class="fw-bold">{{ $jadwal->nama_kegiatan }}</h5>
                                <p class="text-muted small mb-1"><i class="fas fa-map-marker-alt text-danger me-1"></i>
                                    {{ $jadwal->lokasi }}</p>
                                <p class="text-muted small"><i class="fas fa-bullseye text-info me-1"></i> Sasaran:
                                    {{ ucfirst($jadwal->kategori_sasaran) }}</p>
                            </div>
                        </div>
                    </div>
                @empty
